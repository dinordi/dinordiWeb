...
SERVICES = ["pikaraoke-ledfx", "spotify-ledfx"]  # Define services
...
def start_service(service):
    ...
    output = run_command(f"sudo systemctl start {service}.service")
...
def stop_service(service):
    if service not in SERVICES:
        return jsonify({"error": "Invalid service"}), 400
    output = run_command(f"sudo systemctl stop {service}")
...
def status_service(service):
    if service not in SERVICES:
        return jsonify({"error": "Invalid service"}), 400
    command = f"systemctl is-active {service}.service"
